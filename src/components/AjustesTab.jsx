    ...
    <div className="mt-4 grid gap-6 md:grid-cols-2 md:items-start anim-fade-in">
      <Panel titulo="Marca y Configuración Visual">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <div className="flex flex-col">
            <label className="mb-1 block text-xs font-semibold text-slate-500 uppercase tracking-wider">Color Principal</label>
            <input type="color" value={brand.colorPrimario} onChange={(e) => setBrand((prev) => ({ ...prev, colorPrimario: e.target.value }))} className="h-10 w-full cursor-pointer rounded-lg border-none" />
            <button
              onClick={() => setBrand((prev) => ({ ...prev, colorPrimario: MEC_LOGO_PRIMARY_COLOR }))}
              className="mt-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-700 transition hover:bg-slate-50"
            >
              Usar color del logo MEC
            </button>
          </div>
          <div className="flex flex-col">
            <label className="mb-1 block text-xs font-semibold text-slate-500 uppercase tracking-wider">Color Secundario</label>
            <input type="color" value={brand.colorSecundario} onChange={(e) => setBrand((prev) => ({ ...prev, colorSecundario: e.target.value }))} className="h-10 w-full cursor-pointer rounded-lg border-none" />
          </div>

          <div className="sm:col-span-2 mt-2">
            <label className="mb-1 block text-xs font-semibold text-slate-500 uppercase tracking-wider">Logo de la Empresa</label>
            <input type="file" accept="image/*" onChange={manejarLogo} className="w-full rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-xs" />
            {brand.logo && (
              <div className="mt-3 flex flex-col items-start gap-2">
                <div className="p-2 border border-slate-200 rounded-lg bg-slate-50 inline-block">
      ...
      <Panel titulo="Sincronización en la Nube">
        ...
        <div className="space-y-4">
          <div className="flex items-start gap-2 rounded-lg bg-indigo-50/50 p-3 text-sm text-indigo-700">
          ...
          <div className="rounded-lg bg-slate-50 border border-slate-100 p-3 text-xs text-slate-600 font-mono">
            Estado: {estadoNube}
          </div>
      ...
      <Panel titulo="Inteligencia Artificial (Gemini AI)">
        ...
        <div className="space-y-4">
          <div className="flex items-start gap-2 rounded-lg bg-emerald-50/50 p-3 text-sm text-emerald-700 border border-emerald-100">
            <Sparkles size={20} className="shrink-0 mt-0.5" />
            <p>Tu clave API se almacena de manera segura <b>únicamente en tu navegador local</b>.</p>
          </div>
          <div className="flex flex-col">
            <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Clave API de Gemini</label>
            <input 
              type="password" 
              placeholder="AIzaSy..."
              value={brand.geminiApiKey || ""} 
              onChange={(e) => setBrand((prev) => ({ ...prev, geminiApiKey: e.target.value }))} 
              className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm shadow-sm transition-all focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/20"
            />
            <p className="mt-1.5 text-[10px] text-slate-400">Puedes obtener tu clave gratuita en <a href="https://aistudio.google.com/" target="_blank" rel="noreferrer" className="text-emerald-600 underline">Google AI Studio</a>.</p>
          </div>
        </div>
      </Panel>
